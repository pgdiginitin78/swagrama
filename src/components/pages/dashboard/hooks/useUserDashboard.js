...
const useUserDashboard = (user) => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const [countsRes, activitiesRes, opdRes, therapiesRes] = await Promise.all([
        ...
        if (activitiesRes.data.statusCode === 200) {
          setUpcomingActivities(activitiesRes.data.data);
        }
        if (opdRes.data.statusCode === 200) {
          setUpcomingOPD(opdRes.data.data);
        }
        if (therapiesRes.data.statusCode === 200) {
