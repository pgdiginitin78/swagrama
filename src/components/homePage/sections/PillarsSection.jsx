...
const PillarsSection = ({ healersRef, isHealersInView }) => {
  ...
  return (
    <section
      ref={healersRef}
      className="pb-24 px-4 md:px-12 xl:px-6 2xl:px-60"
    >
        ...
        >
          {partnersData.map((healer, index) => (
            <motion.div
              ...
              whileHover="hover"
              className="relative h-[450px] 2xl:h-[700px] rounded-[1rem] overflow-hidden shadow-2xl group cursor-pointer transition-all duration-700"
            >
              <img
                src={healer.image}
                alt={healer.name}
