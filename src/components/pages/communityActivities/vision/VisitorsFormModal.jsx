...
import { Banknote, Calendar, ClipboardList, Info, MapPin, User as UserIcon } from "lucide-react";
...
const schema = yup.object().shape({
  ...
  mobileNumber: yup.string().required("Mobile number is required").matches(/^[0-9]{10}$/, "Must be 10 digits"),
  city: yup.string().required("City is required"),
  appointmentDate: yup.date().typeError("Invalid date").required("Date is required").nullable(),
  noOfPerson: yup.number().typeError("Must be a number").min(1, "At least 1 person").max(20, "Maximum 20 members allowed").required("Required"),
        ...
        <AnimatePresence>
          ...
          >
            ...
            <div className="overflow-y-auto flex-1 p-5 sm:p-7 custom-scrollbar">
              <form onSubmit={handleSubmit(onSubmit)} className="grid lg:grid-cols-12 gap-7">
                ...
                <div className="lg:col-span-7 space-y-6">
                  ...
                  <div className="bg-gradient-to-br from-green-50 to-white p-5 rounded-2xl border border-green-100 shadow-sm">
                    ...
                    <div className="grid grid-cols-2 gap-4 border-t border-green-100 pt-4">
                      <div className="flex items-center gap-2">
                        <AccessTimeIcon sx={{ fontSize: 18, color: "#166534" }} />
                        <div>
                          <p className="text-[10px] text-slate-400 uppercase font-extrabold group">Timings</p>
                          <p className="text-xs font-bold text-slate-700">{serviceDetails.checkIn} - {serviceDetails.checkOut}</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                        <MapPin className="text-green-700 w-4 h-4" />
                        <div>
                          <p className="text-[10px] text-slate-400 uppercase font-extrabold">Location</p>
                          <p className="text-xs font-bold text-slate-700">Swagrama Center</p>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100">
                    <h4 className="text-[10px] font-black text-slate-400 mb-4 uppercase tracking-[0.2em] flex items-center gap-2">
                      <Calendar className="w-3.5 h-3.5" /> Schedule Configuration
                    </h4>
                  ...
                  <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="md:col-span-2">
                        <InputField control={control} name="fullName" label="Full Name *" error={errors.fullName} />
                      </div>
                      <InputField control={control} name="mobileNumber" label="Contact No. *" type="tel" error={errors.mobileNumber} />
                      <InputField control={control} name="email" label="Email Address *" error={errors.email} />
                      <div className="md:col-span-2">
                        <InputField control={control} name="city" label="City *" error={errors.city} />
                      </div>
                    </div>
                  </div>
                ...
                <div className="lg:col-span-5 space-y-6 lg:sticky lg:top-0 h-fit">
                  ...
                  <div className="bg-green-800 rounded-2xl shadow-xl p-6 text-white relative overflow-hidden">
                    ...
                    <div className="flex items-center gap-2 mb-6 border-b border-white/10 pb-4">
                      ...
                      <h3 className="font-bold text-xs tracking-widest uppercase">Bill Summary</h3>
                    </div>
                    
                    <div className="space-y-4">
                      <div className="flex justify-between text-xs font-semibold">
                        <span className="text-green-200">Base Price ({serviceDetails.price})</span>
                      ...
                      <div className="pt-4 border-t border-white/10 flex justify-between items-end">
                        <div className="animate-pulse-slow">
                          <p className="text-[10px] text-green-300 font-bold uppercase tracking-widest mb-0.5">Payable Total</p>
                  ...
                  {/* Summary Notes */}
                   <div className="bg-white p-5 rounded-2xl border border-dashed border-slate-200">
                    <h5 className="text-[10px] font-bold text-slate-400 uppercase mb-3 px-1">Special Benefits</h5>
