    ...
    <div className="flex flex-col h-auto pb-10 bg-white w-full border rounded-lg overflow-hidden shadow-sm">
      <div className="flex items-center justify-between px-3 py-2 border-b border-gray-100 shrink-0">
        <div>
          ...
          <h2 className="text-[12px] font-bold text-[#002a24]">Therapy Details</h2>
        </div>
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="mx-3 mt-3 rounded-xl overflow-hidden border border-gray-100 shadow-sm">
          <div className="relative h-[180px] bg-gradient-to-br from-[#1a2e22] to-[#2e4a38] flex items-center justify-center">
            {selectedBooking.image ? (
              ...
              <ServiceIcon style={{ fontSize: 40, color: "rgba(255,255,255,0.2)" }} />
            )}
          ...
          <div className="bg-white px-3 py-2 flex items-center justify-between gap-2">
            ...
            <span className={`shrink-0 px-2 py-0.5 rounded-full text-[8px] font-bold ${statusStyles[selectedBooking.status] || "bg-gray-100 text-gray-600"}`}>
        ...
        <div className="p-3 space-y-3">
          <div className="grid grid-cols-2 gap-2">
            <DetailRow icon={<AccessTimeIcon />} label="Duration" value={selectedBooking.duration} />
            <DetailRow icon={<ServiceIcon />} label="Intensity" value={selectedBooking.intensity} />
          </div>

          <div className="bg-[#f0f7ee] border border-[#d4e9ce] rounded-lg p-3">
            <p className="text-[7px] font-black uppercase tracking-widest text-[#6a9060] mb-1">Description</p>
            <p className="text-[11px] font-medium text-[#002a24]">{selectedBooking.detail}</p>
          </div>

          <div className="grid grid-cols-2 gap-2">
             <DetailRow icon={<LocationIcon />} label="Facility" value={selectedBooking.facility} />
             <DetailRow icon={<DoctorIcon />} label="Doctor" value={selectedBooking.doctor || "Not Assigned"} />
          </div>
        </div>
      </div>
