  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden">
      <div className="flex justify-between items-center mb-4 px-1 shrink-0">
        <div>
          <h1 className="text-[22px] md:text-[26px] font-semibold py-1 text-[#003d33] tracking-tighter uppercase leading-none">Other Bookings</h1>
          <p className="text-gray-400 text-[9.5px] font-normal uppercase">Manage miscellaneous gift cards and memberships.</p>
        </div>
        <button className="bg-[#003d33] text-white px-4 py-1.5 rounded-full flex items-center gap-2 text-[11.5px] font-normal shadow-sm transition-all active:scale-95">
          <span className="text-[17px] leading-none">+</span> New Entry
        </button>
      </div>
                ...
                <tr key={booking.id} onClick={() => onSelect(booking)} className={`group cursor-pointer transition-all ${isSelected ? 'bg-[#f4f7f6]' : 'hover:bg-gray-50/50'}`}>
                  ...
                  <td className="px-4 py-2 whitespace-nowrap">
                    <div className="flex items-center gap-2.5">
                      <Avatar sx={{ width: 22, height: 22, bgcolor: '#d4edda', color: '#6d9751', fontSize: '10px', fontWeight: 400 }}>A</Avatar>
      ...
      <footer className="py-2 flex items-center justify-between shrink-0 px-1">
        <span className="text-[9px] font-normal text-gray-400 uppercase">Showing 1-{OTHER_DATA.length} of {OTHER_DATA.length} Bookings</span>
